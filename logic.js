...
function clean(raw){
  ...
  t = t.replace(/\s+/g, ' ');
  // Una nota parentetica "(PERMANENTE)" seguita da altro testo (es. "(PERMANENTE) LV 6 E F")
  // è un'annotazione a margine, non l'indicazione operativa: il testo dopo la parentesi
  // è la vera periodicità e va classificato normalmente.
  t = t.replace(/^\(PERMANENTE\)\s*(?=\S)/i, '');
  return t.trim();
}
